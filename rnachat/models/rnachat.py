...
class RNAChat(Blip2Base):
    ...
    def __init__(self,
        ...
        # Set low_resource early so it can be used later
        ...
        # Keep RNA encoder on GPU (Flash Attention requires GPU tensors)
        # We'll use other memory optimizations instead
        if self.low_resource:
            print("🔧 Keeping RNA encoder on GPU (Flash Attention requirement)")
            # RNA encoder will stay on GPU for Flash Attention compatibility
        ...
        if self.low_resource:
            ...
            try:
                ...
                try:
                    ...
                    # Use more aggressive device mapping to spread model across GPU/CPU
                    device_map = {
                    ...
                    for i in range(40):  # Vicuna-13B has 40 layers
                        if i < 20:
                    ...
                    print("✅ Loaded without quantization with CPU offloading")
        else:
            self.llama_model = LlamaForCausalLM.from_pretrained(
                llama_model,
                torch_dtype=torch.bfloat16,
        ...
        print(f"🔍 LoRA Setup: freeze_llama={freeze_llama}")
        if freeze_llama:
            for name, param in self.llama_model.named_parameters():
                param.requires_grad = False
            print("🔒 All LLaMA parameters frozen")
        else:
            ...
            print("🔧 LoRA model created, printing trainable parameters:")
            self.llama_model.print_trainable_parameters()
            
            # Debug: Check if LoRA parameters are trainable
            trainable_count = 0
            total_count = 0
            for name, param in self.llama_model.named_parameters():
                total_count += 1
            ...
            print(f"📊 Parameter Summary: {trainable_count}/{total_count} trainable")
        ...
        print(f"🔍 Projection Layer: freeze_lp={freeze_lp}")
        if freeze_lp:
        ...
        if hasattr(self.llama_model, 'config'):
            self.llama_model.config.use_cache = False  # Disable KV cache for training
        ...
        # Note: Model will be moved to GPU by the training framework
        # We don't move it here to avoid meta tensor issues with CPU offloading
        
        # Add a flag to track if we need to move to GPU
        self._needs_gpu_move = True
    
    def _ensure_gpu_ready(self):
        """Ensure model is ready for GPU training without meta tensor issues"""
        if self._needs_gpu_move and torch.cuda.is_available():
            try:
                # Move RNA encoder to GPU (required for Flash Attention)
                rna_device = next(self.rna_encoder.parameters()).device
                if rna_device.type == 'cpu':
                    print("🔧 Moving RNA encoder to GPU (Flash Attention requirement)...")
                    self.rna_encoder = self.rna_encoder.cuda()
                    print("✅ RNA encoder moved to GPU")
                
                # Move projection layer to GPU
                proj_device = next(self.rinalmo_llama_proj.parameters()).device
                if proj_device.type == 'cpu':
                    print("🔧 Moving projection layer to GPU...")
                    self.rinalmo_llama_proj = self.rinalmo_llama_proj.cuda()
                    print("✅ Projection layer moved to GPU")
                
                self._needs_gpu_move = False
            except Exception as e:
                print(f"⚠️  Could not move to GPU: {e}")
                print("   Continuing with current device setup...")
        
        # Final check: Ensure we have trainable parameters
        total_trainable = sum(p.numel() for p in self.parameters() if p.requires_grad)
        total_params = sum(p.numel() for p in self.parameters())
        print(f"🔍 Final Model Check:")
        print(f"   - Total parameters: {total_params:,}")
        print(f"   - Trainable parameters: {total_trainable:,}")
        print(f"   - Trainable ratio: {100 * total_trainable / total_params:.3f}%")
        ...
        if total_trainable == 0:
            ...
            print("   Check your freeze_llama and freeze_lp settings")
            
            # Debug: Check LoRA parameters specifically
            lora_params = 0
            for name, param in self.named_parameters():
                if 'lora_A' in name or 'lora_B' in name:
                    lora_params += 1
                    print(f"   LoRA param {lora_params}: {name} - requires_grad={param.requires_grad}")
                    if lora_params <= 5:  # Show first 5
                        print(f"      Shape: {param.shape}, Device: {param.device}")
            
            print(f"   Total LoRA parameters found: {lora_params}")
            
            # Try to fix LoRA parameters
            if lora_params > 0:
                print("🔧 Attempting to fix LoRA parameters...")
                for name, param in self.named_parameters():
                    if 'lora_A' in name or 'lora_B' in name:
                ...
                # Recheck
                total_trainable_after = sum(p.numel() for p in self.parameters() if p.requires_grad)
                print(f"   Trainable parameters after fix: {total_trainable_after:,}")
       

    def encode_rna(self, rna):
        # Get the device where LLaMA model parameters are located
        ...
        # RNA encoder is on GPU (Flash Attention requirement)
        rna_device = next(self.rna_encoder.parameters()).device
        if rna_device.type == 'cuda':
            # Both LLaMA and RNA encoder are on GPU
            inputs_rna = inputs
        else:
            # Move inputs to RNA encoder device
            inputs_rna = inputs.to(rna_device)
        ...
        with grad_ctx:
            # Use bfloat16 autocast for better numerical stability
            with torch.cuda.amp.autocast(dtype=torch.bfloat16):
                outputs = self.rna_encoder(inputs_rna)['representation']
        
        # Ensure outputs are on the same device as projection layer
        if outputs.device != llama_device:
            outputs = outputs.to(llama_device)
    ...
    def forward(self, samples):
        # Ensure model is ready for GPU training
        self._ensure_gpu_ready()
        
        # Clear CUDA cache at start of forward pass
        torch.cuda.empty_cache()
