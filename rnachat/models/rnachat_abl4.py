...
import copy
@registry.register_model("rnachat_abl4")

# def get_device_map() -> str:
#     return 'cuda' if torch.cuda.is_available() else 'cpu'

# device = get_device_map()  # 'cpu'
class RNAChatAbl4(Blip2Base):
    ...
    def encode_rna(self, rna):
        ...
        # Ensure RNA encoder is on the same device as LLaMA model
        if next(self.rna_encoder.parameters()).device != llama_device:
            print(f"🔧 Moving RNA encoder from {next(self.rna_encoder.parameters()).device} to {llama_device}")
            self.rna_encoder = self.rna_encoder.to(llama_device)
        
        # Ensure projection layer is on the same device as LLaMA model
        if next(self.rinalmo_llama_proj.parameters()).device != llama_device:
            print(f"🔧 Moving projection layer from {next(self.rinalmo_llama_proj.parameters()).device} to {llama_device}")
            self.rinalmo_llama_proj = self.rinalmo_llama_proj.to(llama_device)
        ...
        with grad_ctx:
            # Use autocast to ensure correct dtype for RiNALMo
